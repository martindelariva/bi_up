...
dbcursor.execute("SELECT submitted_date, delivery_date, customer_id, site_code, total, total_shipping, \
                 order_status, gmv_enabled, batch_id FROM orders ORDER BY submitted_date")
...
temp_sql = f'select batch_id FROM batch_load_ctrl'
dwhcursor.execute(temp_sql)
temp_results = dwhcursor.fetchall()
for temp_result in temp_results:
    batch_ids_ingested.append(temp_result[0])
print(f'Batchs ingested: {batch_ids_ingested}')
...
for record in myresult:
    ...
    if batch_id not in batch_ids_ingested:
        if batch_id not in batch_ids_2_add:
            batch_ids_2_add.append(batch_id)
        """
...
for batch_id_2_add in batch_ids_2_add:
    temp_sql = f'INSERT INTO batch_load_ctrl (batch_id, ingest_date) VALUES ("{batch_id_2_add}", "{batch_time}")'
    print(temp_sql)
    dwhcursor.execute(temp_sql)
